...
func TestQuery1(t *testing.T) {
	...
	t.Run("query with With filter returns the expected results", func(t *testing.T) {
		...
		query := Query1[componentA, QueryOptions[With[componentB], NoOptional, NoReadOnly]]{}
		err = query.PrepareOptions()
		assert.NoError(err)
		query.Exec(&world)
	...
	t.Run("query with Without filter returns the expected results", func(t *testing.T) {
		...
		query := Query1[componentA, QueryOptions[Without[componentB], NoOptional, NoReadOnly]]{}
		err = query.PrepareOptions()
		assert.NoError(err)
		query.Exec(&world)
	...
	t.Run("query with AND filter returns the expected results", func(t *testing.T) {
		...
		query := Query1[componentA, QueryOptions[And[With[componentB], With[componentC]], NoOptional, NoReadOnly]]{}
		err = query.PrepareOptions()
		assert.NoError(err)
		query.Exec(&world)
	...
	t.Run("query with OR filter returns the expected results", func(t *testing.T) {
		...
		query := Query1[componentA, QueryOptions[Or[With[componentB], With[componentC]], NoOptional, NoReadOnly]]{}
		err = query.PrepareOptions()
		assert.NoError(err)
		query.Exec(&world)
	...
	t.Run("queried component can not be mutated if is specified as read-only", func(t *testing.T) {
		...
		expectedValue := 0
		world := NewWorld()
		query := Query1[componentA, QueryOptions[NoFilter, NoOptional, ReadOnly1[componentA]]]{}
		err := query.PrepareOptions()
		assert.NoError(err)
		_, err = Spawn(&world, &componentA{value: 0}, &componentB{})
		assert.NoError(err)
